...
public class Hotmap
{
	...
	public Hotmap(double[][] input, int spreadradius)
	{
	...
	public BufferedImage makeImage(ColourRange colours)
	{		
		switch(colours)
		{
		case BLUE_TO_RED:
		...
		case WHITE_TO_BLACK:
			int[] greycolour = new int[256];
			for(int i=0;i<256;i++)
			{
				Color c = new Color(i,i,i);
				greycolour[i] = c.getRGB();
			}
			return makeImage(greycolour);
		default:
			int[] nullcolour = new int[] {
				Color.white.getRGB(),
				Color.black.getRGB()				
			};
			return makeImage(nullcolour);
		}
	}
